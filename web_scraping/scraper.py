...
def imdb_scraper(id_list):
    ...
    # create DataFrame
    df = make_dataframe(index, movie_id, review, rating, title, username,
                        found_useful_num, found_useful_den, date)
    df = insert_rows(df)
    return df


def make_dataframe(index, movie_id, review, rating, title, username, found_useful_num, found_useful_den, date):
    df = pd.DataFrame(
           {
            'index':index,
            'movie_id':movie_id,
